Outra forma de construir soluções boas para o MCSP é utilizando meta-heurísticas, i.e. algoritmos de otimização que utilizam aleatorização em conjunto com busca local \cite[p.~4]{yang_nature-inspired_2010} \todo{página 4? \cite{yang_nature-inspired_2010}}. Antes de aplicar algum dos vários algoritmos conhecidos, no entanto, é necessário representar as instâncias do problema utilizando uma estrutura de dados eficiente e adequada para tais procedimentos. Encontrar uma boa representação para problemas complexos, como esse, é uma etapa desafiadora, mas que pode simplificar muito a resolução.
...
\begin{figure}
    ...
    \caption{Grafo representando as instâncias (\texttt{abba}, \texttt{abab}) e (\texttt{abab}, \texttt{abba}).}
    \label{fig:grafo}
\end{figure}
